...
setTSpace ref _ tangent bitangent magS magT orient iFace iVert = do
    ...
    let ts = TangentSpace{ tsFace, tsVert, tsTangent, tsBiTangent, tsMagS, tsMagT, tsOrient }
    modifyIORef ref (ts :)
...
main = do
    ...
    let cbGetNumFaces          = getNumFaces mesh
    ...
    let cbs = MikkTSpaceHSCallbacks{ cbGetNumFaces, cbGetNumVerticesOfFace, cbGetPosition, cbGetNormal, cbGetTexCoord, cbSetTSpaceBasic, cbSetTSpace }
